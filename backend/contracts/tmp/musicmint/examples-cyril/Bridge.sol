...
// ::::::::::::::: Feedbacks in readme! :::::::::::::
    ...
    function submit_new_header(Header calldata header) external payable {
        require(msg.value >= relay_fee, "you did not send enough to relay");

        uint256 h = hash_header(header);
        require(h < difficulty_threshold);
        require(!header_is_known(h), "already in our storage");
        require(header_is_known(header.parent), "parent not known");
        require(
            header.height == headers[header.parent].height + 1,
        ...
        if (header.height > best_height) {
            ...
            canon_chain[best_height] = h;
            uint hashParent = header.parent;
            for (uint i = best_height; i > 0; i--) {
                if (canon_chain[i-1] == hashParent) {
                    break;
                }
                canon_chain[i - 1] = hashParent;
                hashParent = headers[hashParent].parent;
            }
        }
        emit HeaderSubmitted(h, header.height, msg.sender);
    }
    ...
    function header_is_known(uint256 header_hash) public view returns (bool) {
        return (headers[header_hash].height != 0 ||
            headers[header_hash].parent != 0 ||
            headers[header_hash].storage_root != 0 ||
            headers[header_hash].transactions_root != 0 ||
            headers[header_hash].pow_nonce != 0 ||
            fee_recipient[header_hash] != address(0));
    }
    ...
    function header_is_canon(uint256 header_hash) public view returns (bool) {
        uint height = headers[header_hash].height;
        return canon_chain[height] == header_hash;
    }

    /// modifier verify condition
    /// to use in both verification function
    modifier verify_condition(uint _hashHead, uint _min_depth) {
        require(
            msg.value >= verify_fee,
            "you didn't send enough my good fella"
        );
        require(
            _min_depth <= best_height,
            "the depth cannot be longer than the chain, or underflow revert"
        );
        require(header_is_known(_hashHead), "header unknown");
        require(header_is_canon(_hashHead), "header is not canon");
        _;
    }
    ...
    ) external payable verify_condition(header_hash, min_depth) returns (bool) {
        (bool succes, ) = fee_recipient[header_hash].call{value: verify_fee}("");
        require(succes, "transaction not done, revert");

        uint height = headers[header_hash].height;
        if (height <= best_height - min_depth) {
            return
                check_merkle_proof(
                    tx_hash,
                    p,
                    headers[header_hash].transactions_root
                );
        }
        return false;
    }
    ...
    function verify_state(
        ...
        if (height <= best_height - min_depth) {
            uint stateHash = uint(
                keccak256(abi.encode(claim.key, claim.value))
            ); // assuming it was this hashing func
            return
                check_merkle_proof(
                    stateHash,
                    p,
